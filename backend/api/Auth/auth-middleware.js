...
const openTo = (role) => (req, res, next) => {
  try {
    if (!role.includes(req.decodedToken.role)) {
...
//req sahibi veya admin mi?
const isOwnerOfRequest = (req, res, next) => {
  try {
    const { user_id, role } = req.decodedToken;
    const { id } = req.params;
    if (id == user_id || role == "Admin") {
      next();
    } else {
      res.status(403).json({ message: "Izinsiz giris" });
    }
  } catch (error) {}
};
...
const loginCheck = async (req, res, next) => {
  try {
    ...
    if (!email || !password) {
      res.status(400).json({ message: "Email ve sifre gereklidir" });
      return;
    } else {
